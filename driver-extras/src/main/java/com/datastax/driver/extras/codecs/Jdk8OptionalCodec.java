 ...
 */
public class Jdk8OptionalCodec<T> extends MappingCodec<Optional<T>, T> {

    private final Predicate<T> isAbsent;

    public Jdk8OptionalCodec(TypeCodec<T> codec) {
        this(codec, isAbsent());
    }
    ...
    public Jdk8OptionalCodec(TypeCodec<T> codec, Predicate<T> isAbsent) {
