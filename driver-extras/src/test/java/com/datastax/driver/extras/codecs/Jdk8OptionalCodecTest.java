...
public class Jdk8OptionalCodecTest extends PerClassSingleNodeCluster {

    private final Jdk8OptionalCodec<List<String>> optionalCodec = new Jdk8OptionalCodec<>(list(varchar()));
    ...
    /**
     * <p>
     * Validates that if a column is unset, that retrieving the value using {@link Jdk8OptionalCodec} should return
    ...
    /**
     * <p>
     * Validates that if a column is set to {@link Optional#empty()} using {@link Jdk8OptionalCodec} that it should be
     * stored as null and that retrieving it should return {@link Optional#empty()} using {@link Jdk8OptionalCodec}.
     * </p>
     *
     * @test_category data_types:serialization
     * @expected_result an absent value.
    ...
    /**
     * <p>
     * Validates that if a column is set to an {@link Optional} value using {@link Jdk8OptionalCodec} that it should be
     * stored as the option's value and that retrieving it should return an {@link Optional} using {@link Jdk8OptionalCodec}
    ...
    public void should_map_a_primitive_type_to_empty() {
        Jdk8OptionalCodec<Long> optionalLongCodec = new Jdk8OptionalCodec<>(bigint());
    ...
    public void should_map_a_nullable_type_to_empty() {
        Jdk8OptionalCodec<BigDecimal> optionalDecimalCodec = new Jdk8OptionalCodec<>(decimal());
