...
public abstract class ParseUtils {
     ...
     * to a valid CQL long literal, {@code false} otherwise.
     */
    ...
    /**
     * Convert from a raw CQL date values in the long literal form
     ...
     */
    public static int parseDaysSinceEpoch(String raw) throws ParseException {
        ...
        long cqlLong;
        try {
            cqlLong = Long.parseLong(raw);
        } catch (NumberFormatException e) {
            throw new ParseException(String.format("Cannot parse date value from \"%s\"", raw), -1);
        }
        if (cqlLong < 0 || cqlLong > MAX_LONG_VALUE)
            throw new ParseException(String.format("Numeric literals for DATE must be between 0 and %d (got %d)",
                MAX_LONG_VALUE, cqlLong), -1);
        return (int)(cqlLong - EPOCH_AS_CQL_LONG);
    }
